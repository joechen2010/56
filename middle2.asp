...
<table cellspacing=0 cellpadding=0 width="100%" border=0>
    ...
    <td style="BORDER-RIGHT: #FF9501 1px solid; BORDER-LEFT: #FF9501 1px solid; BORDER-BOTTOM: #FF9501 1px solid" align="center"> 
      <%  set rs_info=server.CreateObject("adodb.recordset")
				     rs_info.open "select top 32 * from info2 where (province='"&fz&"' or city='"&fz&"' or province2='"&fz&"' or city2='"&fz&"') and infotype='货源信息' order by infoid desc",conn,1,1
	%>
...
wireOpen();
      ...
      <table width=99% height="22" border=0 align="center" cellpadding=0 cellspacing=0 bordercolor="#CCFF99" bgcolor="#999999">
        ...
        <tr <%if i mod 2=0 then response.Write "bgcolor=#FFEAD3" end if%> bgcolor="#FFFFFF">
          ...
          <td width="26%" align="center"  valign=top><%=rs_info("city2")%>&nbsp;<%=rs_info("area2")%></td>
    ...
    <TD vAlign=top width="48%"> 
      <TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
          ...
          <TD style="BORDER-RIGHT: #6BC42C 1px solid; BORDER-LEFT: #6BC42C 1px solid; BORDER-BOTTOM: #6BC42C 1px solid" align="center"> 
            <%  set rs_info=server.CreateObject("adodb.recordset")
				     rs_info.open "select top 32 * from info2 where (province='"&fz&"' or city='"&fz&"' or province2='"&fz&"' or city2='"&fz&"') and infotype='车源信息' order by infoid desc",conn,1,1
				%>
                  <%if rs_info.bof and rs_info.eof then%>
                  <table cellspacing=0 cellpadding=0 width="100%" border=0>
                    <tr> 
              ...
              <tr> 
                <td  valign=top height=100> 
                  <%set rs_h=server.CreateObject("adodb.recordset")
					 rs_h.open "select top 10 * from zx_info where (province='"&fz&"' or city='"&fz&"' or province2='"&fz&"' or city2='"&fz&"') and infotype='货运专线' order by infoid desc",conn,1,1
			  %>
                  <table width="100%" border="0" cellpadding="0" cellspacing="0">
        ...
        <tr> 
          <td class=white12 valign=center height=25 background="images/menu03.jpg"> 
            <table cellspacing=0 width="100%" border=0>
              <tr> 
                <td width="20"> </td>
                <td style="FONT-SIZE: 14px"><b>客运专线</b></td>
                <td align="right"> </td>
                <td align="right"><a  href="pline/index.asp">更多&gt;&gt;</a></td>
              </tr>
            </table>
          </td>
        </tr>
        <tr align=middle> 
          <td style="BORDER-RIGHT: #43A6DD 1px solid; BORDER-LEFT: #43A6DD 1px solid; BORDER-BOTTOM: #43A6DD 1px solid" 
                valign=top> 
            <table cellspacing=0 cellpadding=0 width="100%" border=0>
              <tr> 
                <td  valign=top height=100> 
                  <%  set rs_info=server.CreateObject("adodb.recordset")
				rs_info.open "select top 10 * from zx_info a,qyml b where (a.province='"&fz&"' or a.city='"&fz&"' or a.province2='"&fz&"' or a.city2='"&fz&"') and a.infotype='客运专线' and a.gsid=b.user order by infoid desc",conn,1,1
				%>
                  <%if rs_info.bof and rs_info.eof then%>
                  <table cellspacing=0 cellpadding=0 width="100%" border=0>
                    <tr> 
                  ...
                  <table cellspacing=0 cellpadding=0 border=0 width=100%>
					...
					y=0
					do while not rs_info.eof%>					
                      <td align=center><a href="javascript:openwindow('pline/detail.asp?InfoID=<%=rs_info("infoid")%>',500,400)"><%=rs_info("a.city")%>←→<%=rs_info("city2")%></a></td>
          ...
          <tr align=middle> 
            <td style="BORDER-RIGHT: #EF219D 1px solid; BORDER-LEFT: #EF219D 1px solid; BORDER-BOTTOM: #EF219D 1px solid" valign=top height=140> 
              <table height=67 cellspacing=0 cellpadding=0 width="100%" border=0>
                ...
                <%  set rs_info=server.CreateObject("adodb.recordset")
				     rs_info.open "select top 6 * from qyml where province='"&fz&"' or city='"&fz&"' order by id desc",conn,1,1
				%>	
				<%if rs_info.bof and rs_info.eof then%>			
              <table height=67 cellspacing=0 cellpadding=0 width="100%" border=0> 
                <tr> 
