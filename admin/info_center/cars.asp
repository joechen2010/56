...
<% if request("action")="del" then
   page=request("page")
     ...
     if instr(idlist,",")>0 then
		...
		next
		response.Redirect "cars.asp?page="&page&""
     		else
...
sub deleteannounce(id)
    ...
    sql_del="delete from info2 where infoid="&cstr(id)
    conn.execute sql_del
End sub	
if request("pass")="0" then
  conn.execute("update info2 set pass=false where infoid="&request("id"))
  response.Redirect "?page="&request("page")&""
  elseif request("pass")="1" then
   conn.execute("update info2 set pass=true where infoid="&request("id"))
  response.Redirect "?page="&request("page")&"" 
end if
'delete -----------
'if  Request.QueryString("del")<>"" then
   'delsql="delete from cars where id="&Request.QueryString("del")
  ' conn.execute(delsql)
'end if
'delete end---------------
%>
...
<div align="center"> 
  <table width="96%" border="0" align="center" cellspacing="0" class="px14">
    ...
    <tr>
      <td align="center" bgcolor="#6699ff" height="30"><font color="#FFFFFF"> 车源信息管理</font></td>
    </tr>
  </table>
   <br>
   <form method="post" name="myform" action="cars.asp">
   <% 
	  set rs=Server.Createobject("ADODB.Recordset")
    sql="select  * from info2 where infotype='车源信息' order by addtime desc"
  ...
  <table width="96%" border="0" cellspacing="1">
    <tr bgcolor="#CCCCCC"> 
    <td width="166" height="25" bgcolor="#e1eefd" align="center">出发地</td>
    <td width="147" height="25" bgcolor="#e1eefd" align="center">目的地</td>
    <td width="108" height="25" bgcolor="#e1eefd" align="center">类型</td>
    <td width="128" height="25" bgcolor="#e1eefd" align="center">重量(吨)</td>
    <td width="138" height="25" bgcolor="#e1eefd" align="center">发布日期</td>
	<td width="157" height="25" bgcolor="#e1eefd" align="center">审核</td>
    <td width="104"  bgcolor="#e1eefd" align="center">删除</td>
    </tr>
	 <% for ipage=1 to rs.pagesize%>
    <tr bgcolor="#F6F6F6"  onMouseOut="mOut(this,'#F6F6F6');" onMouseOver="mOvr(this,'#EBFFBB');"> 
    <td align="Right"><a href="javascript:openwindow('../../truck/detail.asp?infoid=<%=rs("InfoID")%>',500,400)"><%=rs("city")%>&nbsp;<%=rs("area")%>→</a></td>
    <td align="Left"><%=rs("city2")%>&nbsp;<%=rs("area2")%></td>
    <td><%=rs("cartype")%></td>
    <td style="font-family:宋体;"><%=rs("carload")%>吨</td>
    <td><%=rs("addtime")%></td>
