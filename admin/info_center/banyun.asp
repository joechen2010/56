...
<% if request("action")="del" then
   page=request("page")
     ...
     if instr(idlist,",")>0 then
		...
		next
		response.Redirect "banyun.asp?page="&page&""
     		else
...
sub deleteannounce(id)
    ...
    sql_del="delete from banyun_info where infoid="&cstr(id)
    conn.execute sql_del
End sub	
if request("pass")="0" then
  conn.execute("update banyun_info set pass=false where infoid="&request("id"))
  response.Redirect "?page="&request("page")&""
  elseif request("pass")="1" then
   conn.execute("update banyun_info set pass=true where infoid="&request("id"))
  response.Redirect "?page="&request("page")&"" 
end if
'delete -----------
'if  Request.QueryString("del")<>"" then
   'delsql="delete from banyun where id="&Request.QueryString("del")
  ' conn.execute(delsql)
'end if
'delete end---------------
%>
...
<div align="center"> 
  <table width="96%" border="0" align="center" cellspacing="0" class="px14">
    ...
    <tr>
      <td align="center" bgcolor="#6699ff" height="30"><font color="#FFFFFF"> 搬运吊装管理</font></td>
    </tr>
  </table>
   <br>
   <form method="post" name="myform" action="banyun.asp">
   <% 
  ...
  <table width="96%" border="0" cellspacing="1">
    <tr bgcolor="#CCCCCC"> 
    <td width="246" height="25" bgcolor="#e1eefd" align="center">标题</td>
    <td width="331" height="25" bgcolor="#e1eefd" align="center">所在省市</td>
    <td width="213" height="25" bgcolor="#e1eefd" align="center">发布日期</td>
   ...
   <tr bgcolor="#F6F6F6"  onMouseOut="mOut(this,'#F6F6F6');" onMouseOver="mOvr(this,'#EBFFBB');"> 
    <td width="246">&nbsp; 
      ...
      <a href="javascript:openwindow('../../banyun/detail.asp?infoid=<%=rs("InfoID")%>',500,400)"><%=left(rs("title"),6)%></a>... 
      <%else%>
      <a href="javascript:openwindow('../../banyun/detail.asp?infoid=<%=rs("InfoID")%>',500,400)"><%=rs("title")%></a> 
      <%end if%>
    </td>
    <td  width="331" align="center">&nbsp;<%=rs("city")%>&nbsp;<%=rs("area")%></td>
    <td width="213" align="center"><%=rs("addtime")%></td>
	 ...
	 <tr><td width="63%" align="right" bgcolor="#e1eefd">	每页显示<font color="#FF0000"><%=rs.pagesize%></font>条&nbsp;&nbsp;	
	     共有<font color="#FF0000"><%=rs.recordcount%></font>条信息	&nbsp;&nbsp;
			...
			if page>1 then
			 ...
			 response.write "首页&nbsp;下一页&nbsp;" 
			end if
			if page<rs.pagecount then
			  response.Write "<a href=?page="&(page+1)&">下一页</a>&nbsp;<a href=?page="&rs.pagesize&">最后一页</a>"
			 else
