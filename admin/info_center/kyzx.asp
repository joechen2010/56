...
<% if request("action")="del" then
   page=request("page")
     ...
     if instr(idlist,",")>0 then
		...
		next
		response.Redirect "kyzx.asp?page="&page&""
     		else
...
sub deleteannounce(id)
    ...
    sql_del="delete from zx_info where infoid="&cstr(id)
    conn.execute sql_del
	conn.execute("delete from zx_info2 where infoid="&cstr(id))
End sub	
if request("pass")="0" then
  conn.execute("update zx_info set pass=false where infoid="&request("id"))
  response.Redirect "?page="&request("page")&""
  elseif request("pass")="1" then
   conn.execute("update zx_info set pass=true where infoid="&request("id"))
  response.Redirect "?page="&request("page")&"" 
end if
'delete -----------
'if  Request.QueryString("del")<>"" then
   'delsql="delete from kyzx where id="&Request.QueryString("del")
  ' conn.execute(delsql)
'end if
'delete end---------------
%>
...
<div align="center"> 
  <table width="96%" border="0" align="center" cellspacing="0" class="px14">
    ...
    <tr>
      <td align="center" bgcolor="#6699ff" height="30"><font color="#FFFFFF"> 客运专线管理</font></td>
    </tr>
  </table>
   <br>
   <form method="post" name="myform" action="kyzx.asp">
   <% 
	  set rs=Server.Createobject("ADODB.Recordset")
    sql="select  * from zx_info a,qyml b where infotype='客运专线' and a.gsid=b.user order by infoid desc"
  ...
  <table width="96%" border="0" cellspacing="1">
    <tr bgcolor="#CCCCCC"> 
    <td width="178" height="25" bgcolor="#e1eefd" align="center">专线名称</td>
    <td width="184" height="25" bgcolor="#e1eefd" align="center">途经(中转)城市</td>
    <td width="144" height="25" bgcolor="#e1eefd" align="center">公司名称</td>
    <td width="173" height="25" bgcolor="#e1eefd" align="center">发布日期</td>
	<td width="148" height="25" bgcolor="#e1eefd" align="center">审核</td>
    <td width="124"  bgcolor="#e1eefd" align="center">删除</td>
    </tr>
	 <% for ipage=1 to rs.pagesize%>
   <tr bgcolor="#F6F6F6"  onMouseOut="mOut(this,'#F6F6F6');" onMouseOver="mOvr(this,'#EBFFBB');"> 
    <td align="Right">
	<a href="javascript:openwindow('../../fline/detail.asp?infoid=<%=rs("InfoID")%>',500,400)">
	<%=rs("a.city")%>←→<%=rs("city2")%></a></td>
    <td align="Left">
	 <%set rs_tj=conn.execute("select city from zx_info2 where infoid="&rs("infoid")&" order by id desc")
	 if not (rs_tj.eof and rs_tj.bof) then
	   do while not rs_tj.eof 
	   response.Write rs_tj("city")&"&nbsp;"
	   rs_tj.movenext
	   loop
	 end if
	 %>
