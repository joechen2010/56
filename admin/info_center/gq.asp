...
<% if request("action")="del" then
   page=request("page")
     ...
     if instr(idlist,",")>0 then
		...
		next
		response.Redirect "gq.asp?page="&page&""
     		else
...
sub deleteannounce(id)
    ...
    sql_del="delete from gq_info where infoid="&cstr(id)
    conn.execute sql_del
End sub
if request("pass")="0" then
  conn.execute("update gq_info set pass=false where infoid="&request("id"))
  response.Redirect "?page="&request("page")&""
  elseif request("pass")="1" then
   conn.execute("update gq_info set pass=true where infoid="&request("id"))
  response.Redirect "?page="&request("page")&"" 
end if	
'delete -----------
'if  Request.QueryString("del")<>"" then
   'delsql="delete from gq where id="&Request.QueryString("del")
  ' conn.execute(delsql)
'end if
'delete end---------------
%>
...
<div align="center"> 
  <table width="96%" border="0" align="center" cellspacing="0" class="px14">
    ...
    <tr>
      <td align="center" bgcolor="#6699ff" height="30"><font color="#FFFFFF"> 供求信息管理</font></td>
    </tr>
  </table>
   <br>
   <form method="post" name="myform" action="gq.asp">
   <% 
	  set rs=Server.Createobject("ADODB.Recordset")
    sql="select  * from gq_info order by infoid desc"
  ...
  <table width="96%" border="0" cellspacing="1">
   ...
   <tr bgcolor="#F6F6F6"  onMouseOut="mOut(this,'#F6F6F6');" onMouseOver="mOvr(this,'#EBFFBB');"> 
    <td width="246">&nbsp; 
      ...
      <a href="javascript:openwindow('../../trade/detail.asp?infoid=<%=rs("InfoID")%>',500,400)"><%=left(rs("title"),6)%></a>... 
      <%else%>
      <a href="javascript:openwindow('../../trade/detail.asp?infoid=<%=rs("InfoID")%>',500,400)"><%=rs("title")%></a> 
      <%end if%>
    </td>
