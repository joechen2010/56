...
set rs=server.createobject("adodb.recordset")
    ...
    response.write "<script>alert('数据不存在！');top.location.href='car_manage.asp';</script>"
    response.end
    end if
	rs.close
	set rs=nothing
if Action="Del" then
    ...
    response.write "<script>alert('数据删除成功！');top.location.href='car_manage.asp';</script>"
    response.end
elseif Action="Update" then
    conn.execute("update info2 Set Addtime='"&Date()&"' Where ProID="&ProID)
	response.write "<script>alert('数据更新成功！');top.location.href='car_manage.asp';</script>"
    response.end
else
