...
strUnit="个信息"
dim province,jobtype,area,area2

        if request("job")<>"" then
	    j=" and job='"&request("job")&"'"
	    end if	
        if request("jobtype")<>"全部" then
	    jtype=" and jobtype='"&request("jobtype")&"'"
	    end if	
    ...
    sql="select  * from zp_info where 1=1"&c&j&itype&jtype&"  order by infoid desc"
	sfilename="search.asp?province="&request("province")&"&city="&request("city")&"&jobtype="&request("jobtype")&"&job="&request("job")&"&infotype="&request("infotype")&""
...
%>  
	...
	<tr align="Center" style="color:#003399;background-color:White;">		
      <td width="130"><%=rs("infotype")%></td>
      <td  width="299" align="center"><%=rs("province")%>&nbsp;<%=rs("city")%></td>
      <td width="182" align="center"><%=rs("contact")%></td>
